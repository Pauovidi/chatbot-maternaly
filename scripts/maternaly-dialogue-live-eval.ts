...
if (!["understanding", "conversation"].includes(suite) || ![offset, count, repeat].every(Number.isInteger) || offset < 0 || count < 1 || count > 40 || repeat < 1 || repeat > 3) throw new Error("Invalid bounded evaluation arguments");
...
async function main() {
  ...
  for (let run = 1; run <= repeat; run++) {
    const cases = suite === "understanding" ? DIALOGUE_EVALUATIONS : CONVERSATION_EVALUATIONS;
  ...
  const path = `/tmp/maternaly-dialogue-${suite}-${offset}.json`;
