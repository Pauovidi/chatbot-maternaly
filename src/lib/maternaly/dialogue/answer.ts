...
export function dialogueFacts(dialogue: DialogueUnderstanding) {
  ...
  return ids.flatMap((id) => {
    ...
    return [service.summary, ...service.details, ...(service.pricing ?? [])]
...
export function validateDialogueAnswer(raw: unknown, facts: ReturnType<typeof dialogueFacts>): string | undefined {
  ...
  if (/https?:|www\.|@|\b(?:reserva(?:da)?|plaza|inscripci[oó]n)\s+(?:ya\s+)?confirmad|\b(?:he|hemos)\s+(?:reservado|confirmado|cancelado)|\b(?:contrase[nñ]a|tarjeta|diagn[oó]stico|medicaci[oó]n)\b/i.test(text)) return undefined;
