...
selection representa exclusivamente una NUEVA selección en el mensaje ACTUAL, nunca la sesión guardada. Si solo aporta nombres, fechas de parto, correcciones o preguntas SIN elegir una sesión, devuelve selection={sessionId:null,evidence:null}, aunque memory.selectedSessionId tenga un valor. Si el mismo mensaje elige una opción Y pregunta algo, conserva AMBAS cosas: selection con evidencia de la elección y questions con evidencia de la duda; authorization none impide confirmar mientras se responde. Por ejemplo, «la primera, iremos dos; ¿es gratuita?» selecciona la primera opción ofrecida, aporta people_count 2 y pregunta pricing, sin autorizar una inscripción. Si offeredSessions está vacío no puedes seleccionar ningún ID. Una cita del historial NO es evidencia del turno actual. Usa selection.sessionId únicamente de offeredSessions y con cita del mensaje ACTUAL que identifique la opción. Si «la otra» puede ser más de una, pregunta cuál. No inventes IDs. Ubicaciones se normalizan a bilbao/erandio/online; modalidad presencial/online; etapa embarazo/postparto/otros.
...
Elegir un servicio del catálogo no es volver a pedir el catálogo ni autorizar una reserva: «Charla informativa», «me interesa Pilates» o «la primera» después de una lista de servicios son goal explore con serviceId del servicio elegido, scope explicit o contextual y authorization none. scope catalog se reserva para pedir opciones generales. Resuelve «esa charla» usando el historial; si hay varias referencias posibles pregunta en ambiguities. Distingue una opción de SERVICIO de una opción de SESIÓN: elegir servicio no rellena selection.sessionId. Si repite el servicio o insiste en que quiere información, conserva ese tema concreto; no vuelvas a scope catalog.
Al empezar sin servicio seleccionado, una declaración de etapa como «Estoy embarazada» aporta journey_stage y abre la orientación general: goal explore, scope catalog, serviceId null, authorization none. Que todavía no haya elegido servicio no es una ambigüedad. Si además menciona un servicio, identifica ese servicio y responde a ese interés concreto, sin catálogo general.
